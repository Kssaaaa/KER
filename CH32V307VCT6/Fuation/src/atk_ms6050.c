...
uint8_t atk_ms6050_write(uint8_t addr,uint8_t reg, uint8_t len, uint8_t *dat)
{
    ...
    Driver_I2C2_Start();
    Driver_I2C_SendByte((addr << 1) | 0);
    if (Driver_I2C2_WaitAck() == 1)
    {
        Driver_I2C2_Stop();
        return ATK_MS6050_EACK;
    }
    Driver_I2C_SendByte(reg);
    if (Driver_I2C2_WaitAck() == 1)
    {
        Driver_I2C2_Stop();
        return ATK_MS6050_EACK;
...
uint8_t atk_ms6050_read(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *dat)
{
    ...
    while (len)
    {
        *dat = Driver_I2C_ReadByteAck((len > 1) ? 1 : 0);
        len--;
        dat++;
    }
    Driver_I2C2_Stop();
