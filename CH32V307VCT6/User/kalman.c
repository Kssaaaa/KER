...
void Angle_Calcu(void)	 
{
	...
	Accel_x = GetData(ACCEL_XOUT_H); //x轴加速度值获取
	Accel_y = GetData(ACCEL_YOUT_H); //y轴加速度值获取
	Accel_z = GetData(ACCEL_ZOUT_H); //z轴加速度值获取
	Gyro_x  = GetData(GYRO_XOUT_H);  //x轴陀螺仪值获取
	Gyro_y  = GetData(GYRO_YOUT_H);  //y轴陀螺仪值获取
	Gyro_z  = GetData(GYRO_ZOUT_H);  //z轴陀螺仪值获取
	temperature = -158.53+((double)GetData(TEMP_OUT_H))/340.0;//得到温度值
